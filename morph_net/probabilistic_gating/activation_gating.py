...
tricks (go/logistic-sigmoid-trick). The gating probability is trainable.
...
class LogisticSigmoidGating(tf.keras.layers.Layer):
  """Keras layer for stochastic gating using the logistic-sigmoid trick.
    ...
    The gating decision for each channel is independently done.
    For stability the op works in the log odds space: log(p / 1-p).
  """
  ...
  def __init__(self,
    ...
    Args:
      ...
      temp_decay_num_steps: (Optional) Integer. If provided, the temperature is
        exponentially decayed by a factor of 0.1 every `temp_decay_num_steps`.
    ...
    self.temp_decay_num_steps = temp_decay_num_steps
    ...
    if self.temp_decay_num_steps:
      if self.global_step is None:
        raise ValueError('Must provide global_step if decaying temperature.')
      self.temperature = self.temperature * (
          0.1 ** (self.global_step / float(self.temp_decay_num_steps)))
...
def logistic_sigmoid_gating(activation,
  ...
  layer_fn = LogisticSigmoidGating(
      ...
      temp_decay_num_steps=temp_decay_num_steps,
...
def _logistic_sigmoid_sample(logits, tau, straight_through):
  ...
  logistic_sample = logistic_dist.sample(
